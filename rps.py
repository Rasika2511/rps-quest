...
def decide_winner(user: str, cpu: str):
    ...
    return (
        "win"
        if (user, cpu) in winning_pairs
        else "draw"
        if user == cpu or (user, cpu) in draw_pairs
        else "lose"
    )
...
print(
    f"In first game you {results[0]}. In second game you {results[1]}. In third game you {results[2]}."
)
